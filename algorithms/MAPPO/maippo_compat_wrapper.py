"""
Compatibility Wrapper for MAIPPO with Device Churn

MAIPPO = Multi-Agent Independent PPO (Local Critic)
On-policy algorithm with independent (local) value functions.
...
class SimpleMAPPOAgent:
    ...
    def _build_critic(self):
        """Build local (independent) critic network"""
        return torch.nn.Sequential(
            torch.nn.Linear(self.state_dim, 256),
            torch.nn.ReLU(),
            torch.nn.Linear(256, 256),
    ...
    def select_action(self, state: np.ndarray, deterministic: bool = False):
        """Select action (MAIPPO uses local state for critic)"""
        ...
        with torch.no_grad():
            ...
            # Compute value (using local state, not centralized)
    ...
    def update(self, ppo_epochs: int = 4) -> Dict[str, float]:
        ...
        for _ in range(ppo_epochs):
            ...
            # Value loss (using local observations)
...
class MAIPPOCompatAgent:
    """Compatibility wrapper for MAIPPO (Independent PPO) with device churn"""
    ...
    def __init__(
        ...
        """Initialize MAIPPO compatibility wrapper"""
        ...
        # Create simplified MAIPPO agent (uses local critic)
        self.maippo = SimpleMAPPOAgent(
            state_dim=state_dim,
    ...
    def select_actions(
        ...
        # Get masks (MAIPPO doesn't use centralized state)
        masks = np.stack([
            adapted_obs[mid]['mask']
            for mid in self.manager_ids
        ], axis=0)
        
        # Select actions for each manager (independent)
        padded_actions = {}
        for i, manager_id in enumerate(self.manager_ids):
            state = adapted_obs[manager_id]['obs_vec']
            
            action, log_prob, value = self.maippo.select_action(
                state, deterministic=not explore
            )
    ...
    def store_transition(
        ...
        for manager_id in self.manager_ids:
            if hasattr(self, '_temp_data') and manager_id in self._temp_data:
                ...
                self.maippo.rollout_buffer.push(
                    temp['obs'],
                    temp['action'],
                    temp['log_prob'],
                    temp['value'],
    ...
    def update(self) -> Dict[str, float]:
        """Update MAIPPO policy"""
        return self.maippo.update(ppo_epochs=4)
    
    def get_buffer_size(self) -> int:
        """Get rollout buffer size"""
        return len(self.maippo.rollout_buffer)
    
    def save(self, path: str):
        """Save model"""
        torch.save({
            'actor': self.maippo.actor.state_dict(),
            'critic': self.maippo.critic.state_dict(),
            'optimizer': self.maippo.optimizer.state_dict()
        }, path)
    
    def load(self, path: str):
        """Load model"""
        checkpoint = torch.load(path, map_location=self.device)
        self.maippo.actor.load_state_dict(checkpoint['actor'])
        self.maippo.critic.load_state_dict(checkpoint['critic'])
        self.maippo.optimizer.load_state_dict(checkpoint['optimizer'])


if __name__ == "__main__":
    print("Testing MAIPPOCompatAgent...")
    
    manager_ids = ['manager_1', 'manager_2', 'manager_3', 'manager_4']
    agent = MAIPPOCompatAgent(
        manager_ids=manager_ids,
        N_max=44,
        x_dim=6,
        g_dim=26,
