"""
EA Algorithm Test: 4Manager Environment with High Severity, MIDF Churn

This test validates EA's learning capability under heavy device churn:
- Environment: 4 Managers
- Churn Severity: High (10.0%-15.0% devices change)
...
# ============== Churn Parameters ==============
CHURN_ENABLED = True
CHURN_SEVERITY = "high"  # low/mid/high
...
def create_agent(num_managers: int):
    ...
    agent = EAAgent(
        ...
        advantage_tau=ADVANTAGE_TAU,  # Per-device advantage weighting temperature
    ...
    for episode in range(num_episodes):
            ...
            if global_step >= WARMUP_EPISODES * TIME_HORIZON:
                ...
                if losses is not None and losses:
                    ...
                    if 'actor_loss' in losses:  # Only present on policy_delay steps
                        ...
                        # Also collect advantage metrics (only when actor updates)
        ...
        if mini_log:
            ...
            print(f" {status} Reward: {avg_reward:7.1f} | "
                  ...
                  f"Adv: {mean_advantage:6.3f}±{advantage_std:.3f}")
...
def main():
    ...
    logger.info(f"  Advantage tau: {ADVANTAGE_TAU} (softmax temperature)")
