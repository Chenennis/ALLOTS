"""
EA Ablation Test: 10Manager + High Churn + w/o Pair-Set Critic
=============================================================

Ablation variant: Separate state-action encoding (NO Pair-Set)
- Uses separate encoders for state and action, then combines
- Tests if explicit per-device state-action pairing is necessary
...
def create_agent(num_managers: int):
    """Create ABLATION Agent: w/o Pair-Set Critic"""
    agent = EAAgentNoPairSet(
        x_dim=X_DIM,
        g_dim=G_DIM,
        p=P,
        N_max=N_MAX,
