"""
MAPPO Baseline Test: 10 Managers Environment with Mid Severity, Mid Frequency Churn

This test validates MAPPO's performance under device churn using compatibility layer.
...
# ============== Output Parameters ==============
OUTPUT_DIR = f"Test/results/mappo_{ENV_TYPE}_{CHURN_SEVERITY}_{CHURN_FREQUENCY}"
SAVE_CSV = True
SAVE_JSON = True
...
def train(env, agent, num_episodes, logger, output_dir, mini_log=False):
    """Train MAPPO agent"""
    logger.info("=" * 80)
    logger.info(f"Starting MAPPO Training: {num_episodes} episodes")
    ...
    if mini_log:
        print("\n" + "=" * 80)
        print(f">>> MAPPO Training: {num_episodes} episodes | Env: {ENV_TYPE} | Churn: {CHURN_SEVERITY}/{CHURN_FREQUENCY}")
        print("=" * 80)
...
def main():
    ...
    logger.info("\nCreating MAPPO agent with compatibility layer...")
    agent = MAPPOCompatAgent(
        ...
        g_dim=G_DIM,
        p=P,
        lr=LR_ACTOR,  # MAPPO uses single 'lr' parameter
        gamma=GAMMA,
        device=device
    )
    logger.info("MAPPO agent created successfully")
