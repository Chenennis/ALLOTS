"""
MAIPPO Baseline Test: 10 Managers Environment with Mid Severity, Mid Frequency Churn

This test validates MAIPPO's performance under device churn using compatibility layer.

- Algorithm: MAIPPO (Multi-Agent Independent PPO)
- Environment: 10 Managers
- Churn Severity: Mid (10-15% devices change)
- Churn Frequency: Mid (every 10 episodes)
- Training: 1000 episodes (for baseline comparison with EA)
...
N_MAX = 100  # For 10manager (conservative estimate)  # Maximum devices in 4manager environment
...
# ============== Output Parameters ==============
OUTPUT_DIR = f"Test/results/maippo_{ENV_TYPE}_{CHURN_SEVERITY}_{CHURN_FREQUENCY}"
SAVE_CSV = True
SAVE_JSON = True
...
def train(env, agent, num_episodes, logger, output_dir, mini_log=False):
    """Train MAIPPO agent"""
    logger.info("=" * 80)
    logger.info(f"Starting MAIPPO Training: {num_episodes} episodes")
    ...
    if mini_log:
        print("\n" + "=" * 80)
        print(f">>> MAIPPO Training: {num_episodes} episodes | Env: {ENV_TYPE} | Churn: {CHURN_SEVERITY}/{CHURN_FREQUENCY}")
        print("=" * 80)
    ...
    for episode in range(num_episodes):
        ...
        actor_loss = 0.0
        critic_loss = 0.0
        if global_step >= WARMUP_EPISODES * TIME_HORIZON and agent.get_buffer_size() >= 32:
            losses = agent.update()
            if losses:
                actor_loss = losses.get('policy_loss', 0.0)  # PPO returns 'policy_loss'
                critic_loss = losses.get('value_loss', 0.0)
...
def main():
    ...
    logger.info(f"  Algorithm: MAIPPO")
    ...
    logger.info("\nCreating MAIPPO agent with compatibility layer...")
    agent = MAIPPOCompatAgent(
        ...
        g_dim=G_DIM,
        p=P,
        lr=LR_ACTOR,  # MAIPPO uses single 'lr' parameter
        gamma=GAMMA,
        device=device
    )
    logger.info("MAIPPO agent created successfully")
